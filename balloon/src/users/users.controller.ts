...
export class UsersController {
  ...
  @Get('update-tuition-fee')
  async updateTuitionFee() {
    await this.usersService.updateAllTuitionFees();
  }
  ...
  findOne(@Param('id') id: number): Promise<NullableType<User>> {
    return this.usersService.findOneCalculate(id);
  }
  ...
  async updateTeacher(@Req() request) {
    const { id } = this.commonService.getAccountInformationLogin(request);
    await this.usersService.updateTeacher(id);
  ...
  @Patch(':id')
  @HttpCode(HttpStatus.OK)
  async update(@Req() request, @Param('id') accountId: number, @Body() updateProfileDto: UpdateUserDto) {
    const { id } = this.commonService.getAccountInformationLogin(request);
